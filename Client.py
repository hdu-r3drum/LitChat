...
def Person_Message(sock, choice):
    ...
    # print(md5.hexdigest())
    text = str(choice) + '  ' + name + '  ' + str(md5.hexdigest())
...
def main():
    ...
    while True:
        ...
        # signal标识注册或者登陆时返回的值
        data, name, address = Person_Message(sock, choice)
        # PersonMsg = Person_Message(sock, choice)
        if data[1] == 'OK':
            # os.system("clear")
            print('You have successfully entered the room\t\t')
            break
        elif data[1] == 'Error_UserExist':
            print('User already exists!')
        elif data[1] == 'Error_PasswordError':
            print('Password error!')
        elif data[1] == 'Error_UserNotExist':
